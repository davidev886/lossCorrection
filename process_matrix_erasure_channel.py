...
#    print("YL", qu.expect(1j * XL * ZL, rho_L))   
    ...
    for data_q in list_qubits:
        ...
        #apply Rloss with an angle phi from the state 1


        if outcomes_ancilla_1[data_q] == 0:  #ancilla1 in 0 state
            prob_outcome_1 = (rho_L * Pp_ancilla).tr()
            if abs(prob_outcome_1.imag) > 1e-5: print("warning: im prob_outcome_1 = {prob_outcome_1}")
            if prob_outcome_1 == 0:
                #the state cannot be projected in the +1 eigenstate of the ancilla
                null_state = True
            else:
                rho_L = Pp_ancilla * rho_L * Pp_ancilla.dag() / abs(prob_outcome_1)
                #perform the rotation from 1 and the second measurement
                rho_L = rotation_ops_1[data_q] * rho_L * rotation_ops_1[data_q].dag()    
                rho_L = apply_qnd_process_unit(chi_matrix, rho_L, data_q, chi_threshold)                                                        
                if outcomes_ancilla_2[data_q] == 0:  #ancilla2 in 0 state                
                    prob_outcome_2 = (rho_L * Pp_ancilla).tr()                    
                    if abs(prob_outcome_2.imag) > 1e-5: print("warning: im prob_outcome_2 = {prob_outcome_2}")
                    if prob_outcome_2 == 0:
                        #the state cannot be projected in the +1 eigenstate of the ancilla
                        null_state = True
                    else:
                        rho_L = Pp_ancilla * rho_L * Pp_ancilla.dag() / abs(prob_outcome_2)                        
                elif outcomes_ancilla_2[data_q] == 1:  #ancilla2 in 1 state                
                    prob_outcome_2 = (rho_L * Pm_ancilla).tr()                    
                    if abs(prob_outcome_2.imag) > 1e-5: print("warning: im prob_outcome_2 = {prob_outcome_2}")
                    if prob_outcome_2 == 0:
                        #the state cannot be projected in the +1 eigenstate of the ancilla
                        null_state = True
                    else:
                        rho_L = Pm_ancilla * rho_L * Pm_ancilla.dag() / abs(prob_outcome_2)                        
                        rho_L = Xa  * rho_L * Xa.dag() #reinitializing ancilla                               
            prob_outcome = prob_outcome_1 * prob_outcome_2
            print(prob_outcome_1, prob_outcome_2, prob_outcome)

        elif outcomes_ancilla_1[data_q] == 1:   #ancilla in 1 state #do only one ancilla measurement
